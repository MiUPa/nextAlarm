...
class _AlarmCardState extends State<_AlarmCard>
  ...
  Widget build(BuildContext context) {
    ...
    final pauseSummary = _getPauseSummary(l10n, widget.alarm);
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        // Challenge
                        if (widget.alarm.challenge !=
                            models.WakeUpChallenge.none)
                          Row(
                            children: [
                              Icon(
  ...
  String _getLocalizedRepeatText(AppLocalizations l10n, models.Alarm alarm) {
    ...
    if (alarm.repeatDays.containsAll(weekdays) &&
        alarm.repeatDays.length == 5) {
      return l10n.repeatWeekdays;
    }

    if (alarm.repeatDays.containsAll(weekends) &&
    ...
    final orderedDays = alarmSettings.orderedWeekdays(alarm.repeatDays);
    return orderedDays.map((d) => dayNames[d]!).join(' ');
  }

  String? _getPauseSummary(AppLocalizations l10n, models.Alarm alarm) {
    if (alarm.pausedDates.isEmpty) return null;

    final sortedPausedDates =
        alarm.pausedDates
            .map(models.alarmDateFromKey)
            .whereType<DateTime>()
            .toList()
          ..sort();
    if (sortedPausedDates.isEmpty) return null;

    final tomorrowKey = models.alarmDateKey(
      DateTime.now().add(const Duration(days: 1)),
    );
    if (alarm.pausedDates.contains(tomorrowKey)) {
      return l10n.pauseSummaryTomorrow;
    }
    if (sortedPausedDates.length == 1) {
      final dateLabel = MaterialLocalizations.of(
        context,
      ).formatShortDate(sortedPausedDates.first);
      return l10n.pauseSummaryOne(dateLabel);
    }
    return l10n.pauseSummaryMany(sortedPausedDates.length);
  }
