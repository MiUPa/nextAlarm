...
class _AlarmRingingScreenState extends State<AlarmRingingScreen>
  ...
  void _initShakeChallenge() {
    final difficulty = widget.alarm.challengeDifficulty;
    _requiredShakes =
        5 + (difficulty * 10); // 15/25/35 shakes based on difficulty
    ...
    if (!_isListening) {
      ...
      await _speech.listen(
        onResult: (result) {
          ...
          if (_recognizedText.toLowerCase().contains(
            _targetPhrase.toLowerCase(),
          )) {
            _stopAlarm();
          }
        },
  ...
  Widget _buildStepChallenge() {
    final l10n = AppLocalizations.of(context)!;
    final progress = _requiredSteps > 0 ? _currentSteps / _requiredSteps : 0.0;
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Text(
  ...
  String _challengeLabel(AppLocalizations l10n) {
    switch (widget.alarm.challenge) {
      case WakeUpChallenge.math:
  ...
  String _difficultyLabel(AppLocalizations l10n) {
    switch (widget.alarm.challengeDifficulty) {
      case 1:
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context)!;
    final trimmedLabel = widget.alarm.label.trim();
                    ...
                    Wrap(
                      ...
                      children: [
                        _StatusPill(
                          ...
                          label: l10n.completeChallengeToStopAlarm,
                        ),
                        _StatusPill(
                          icon: _challengeIcon(),
                          label:
                              '${_challengeLabel(l10n)}  ${_difficultyLabel(l10n)}',
                        ),
                      ],
                    ),
